...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--celiums-hybrid-auto"},
        "use performance cores for single-token work and all available cores for batch work",
        [](common_params & params) {
            bool allowed_mask[GGML_MAX_N_THREADS] = { false };
            int allowed_count = 0;
            if (!common_cpu_get_allowed_mask(allowed_mask, allowed_count)) {
                return;
            }

            bool performance_mask[GGML_MAX_N_THREADS] = { false };
            int pcore_count = 0;
            if (!common_cpu_get_performance_mask(performance_mask, pcore_count)) {
                std::copy(std::begin(allowed_mask), std::end(allowed_mask), std::begin(performance_mask));
                pcore_count = allowed_count;
            }
            std::copy(std::begin(performance_mask), std::end(performance_mask), std::begin(params.cpuparams.cpumask));
            params.cpuparams.mask_valid = true;
            params.cpuparams.strict_cpu = true;
            params.cpuparams.n_threads = pcore_count;

            std::copy(std::begin(allowed_mask), std::end(allowed_mask), std::begin(params.cpuparams_batch.cpumask));
            params.cpuparams_batch.n_threads = allowed_count;
            params.cpuparams_batch.mask_valid = true;
            params.cpuparams_batch.strict_cpu = false;
        }
    ));
    add_opt(common_arg(
        {"-tb", "--threads-batch"}, "N",
