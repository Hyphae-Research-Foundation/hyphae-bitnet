...
bool common_cpu_get_performance_mask(bool (&mask)[GGML_MAX_N_THREADS], int & count) {
    if (!is_hybrid_cpu()) {
        return false;
    }
    cpu_set_t affinity;
    ...
    bool detected[GGML_MAX_N_THREADS] = { false };
    int detected_count = 0;
    const int n_cpu = std::min<int>(sysconf(_SC_NPROCESSORS_ONLN), GGML_MAX_N_THREADS);
    for (int cpu = 0; cpu < n_cpu; ++cpu) {
        if (!CPU_ISSET(cpu, &affinity) || pin_cpu(cpu)) {
            continue;
        }
        if (!is_running_on_efficiency_core()) {
            detected[cpu] = true;
    ...
    std::copy(std::begin(detected), std::end(detected), std::begin(mask));
    count = detected_count;
    return true;
}

bool common_cpu_get_allowed_mask(bool (&mask)[GGML_MAX_N_THREADS], int & count) {
    cpu_set_t affinity;
    if (pthread_getaffinity_np(pthread_self(), sizeof(affinity), &affinity)) {
        return false;
    }
...
int32_t common_cpu_get_num_allowed() {
    cpu_set_t affinity;
    if (pthread_getaffinity_np(pthread_self(), sizeof(affinity), &affinity)) {
...
bool common_cpu_get_allowed_mask(bool (&mask)[GGML_MAX_N_THREADS], int & count) {
    std::fill(std::begin(mask), std::end(mask), false);
    count = std::min<int>(std::thread::hardware_concurrency(), GGML_MAX_N_THREADS);
...
#endif
    return std::min(common_cpu_get_num_physical_cores(), common_cpu_get_num_allowed());
}
...
struct common_init_result::impl {
    ...
    decltype(ggml_threadpool_free) * threadpool_free = nullptr;
    ...
    ~impl() {
        ...
        if (threadpool_free) {
            ...
            threadpool_free(threadpool_batch);
        }
    }
};
...
common_init_result::common_init_result(common_params & params, bool model_only) :
    ...
    auto * threadpool_new = reg ? (decltype(ggml_threadpool_new) *) ggml_backend_reg_get_proc_address(reg, "ggml_threadpool_new") : nullptr;
    pimpl->threadpool_free = reg ? (decltype(ggml_threadpool_free) *) ggml_backend_reg_get_proc_address(reg, "ggml_threadpool_free") : nullptr;
    if (threadpool_new && pimpl->threadpool_free) {
        auto tpp = ggml_threadpool_params_from_cpu_params(params.cpuparams);
        auto tpp_batch = ggml_threadpool_params_from_cpu_params(params.cpuparams_batch);
        const bool pools_match = ggml_threadpool_params_match(&tpp, &tpp_batch);
        if (!pools_match) {
            pimpl->threadpool_batch = threadpool_new(&tpp_batch);
            tpp.paused = true;
        }
        pimpl->threadpool = threadpool_new(&tpp);
        if (pimpl->threadpool && (pools_match || pimpl->threadpool_batch)) {
            llama_attach_threadpool(lctx, pimpl->threadpool, pimpl->threadpool_batch);
        } else {
            COM_ERR("%s", "failed to create CPU threadpools\n");
            pimpl->context.reset();
            return;
        }
    }
