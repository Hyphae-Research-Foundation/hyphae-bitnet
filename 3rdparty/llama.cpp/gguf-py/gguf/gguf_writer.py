    ...
    def add_tensor_info(
        ...
        if raw_dtype is None:
            ...
            if raw_dtype == GGMLQuantizationType.I2_S:
                if len(tensor_shape) != 2 or any(dim <= 0 for dim in tensor_shape) or tensor_shape[-1] % 128 != 0:
                    raise ValueError(f"I2_S requires rank 2 and row width divisible by 128, got {tuple(tensor_shape)}")
                expected_nbytes = prod(tensor_shape) // 4 + 32
                if tensor_dtype != np.uint8 or tensor_nbytes != expected_nbytes:
                    raise ValueError(
                        f"I2_S tensor requires {expected_nbytes} uint8 bytes, got {tensor_nbytes} {tensor_dtype}"
                    )
