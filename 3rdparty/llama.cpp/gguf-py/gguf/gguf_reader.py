    ...
    def _build_tensors(self, start_offs: int, fields: list[ReaderField]) -> None:
        ...
        for field in fields:
            ...
            if ggml_type == GGMLQuantizationType.F16:
                ...
                if ggml_type in (GGMLQuantizationType.I2_S, GGMLQuantizationType.TL1):
                    actual_bytes = n_elems // 4 + 32
                    item_count = actual_bytes
                    n_bytes = actual_bytes
                    np_dims = (actual_bytes,)
                elif ggml_type == GGMLQuantizationType.TL2:
                    ne0, ne1 = int(dims[0]), int(dims[1])
                    actual_bytes = (ne0 - 256) * ne1 // 3 * 5 // 8 + 256 * ne1 // 2 * 4 // 8
                    actual_bytes = (actual_bytes + 31) // 32 * 32 + 32
                    item_count = actual_bytes
                    n_bytes = actual_bytes
                    np_dims = (actual_bytes,)
                else:
