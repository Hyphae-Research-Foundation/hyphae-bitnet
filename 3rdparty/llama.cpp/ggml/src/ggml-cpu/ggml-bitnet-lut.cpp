...
bool ggml_bitnet_can_mul_mat(const struct ggml_tensor * src0, const struct ggml_tensor * src1, const struct ggml_tensor * dst) {
    return is_type_supported(src0->type) && src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32 && src1->ne[1] <= 1;
}
...
size_t ggml_bitnet_mul_mat_get_wsize(const struct ggml_tensor * src0, const struct ggml_tensor * src1, const struct ggml_tensor * dst) {
    ...
    size_t wsize = ne10 * ne11 * 15 * sizeof(int8_t) + ne11 * 2 * sizeof(bitnet_float_type);
...
void ggml_bitnet_mul_mat(const struct ggml_compute_params * params, struct ggml_tensor * dst) {
    ...
    bitnet_float_type * lut_biases = lut_scales + ne11;
    if (ith == 0) {
        ggml_bitnet_mul_mat_task_init(src1->data, qlut, lut_scales, lut_biases, ne10, ne00, ne11, bits);
    }
    if (nth > 1) {
        ggml_barrier(params->threadpool);
    }
    ggml_bitnet_mul_mat_task_compute(extra->qweights, extra->scales, qlut, lut_scales, lut_biases, dst->data, ne10, ne00, ne11, bits);
}
...
bool ggml_bitnet_can_mul_mat(const struct ggml_tensor * src0, const struct ggml_tensor * src1, const struct ggml_tensor * dst) {
    return is_type_supported(src0->type) && src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32;
}
