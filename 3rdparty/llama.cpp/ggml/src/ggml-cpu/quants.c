...
static inline __m256i roundf_i32_8(const __m256 v) {
    const __m256i truncated = _mm256_cvttps_epi32(v);
    const __m256 fraction = _mm256_sub_ps(v, _mm256_cvtepi32_ps(truncated));
    const __m256 abs_fraction = _mm256_andnot_ps(_mm256_set1_ps(-0.0f), fraction);
    const __m256i round_mask = _mm256_castps_si256(
        _mm256_cmp_ps(abs_fraction, _mm256_set1_ps(0.5f), _CMP_GE_OQ));
    const __m256i sign = _mm256_srai_epi32(_mm256_castps_si256(v), 31);
    const __m256i direction = _mm256_or_si256(sign, _mm256_set1_epi32(1));
    return _mm256_add_epi32(truncated, _mm256_and_si256(round_mask, direction));
}
#endif
...
void quantize_row_i8_s(const float * x, void * y, int64_t n, float* act_scales, int32_t* act_sums) {
    ...
    for (; i + 8 <= n; i += 8) {
        ...
        max_abs = _mm256_max_ps(max_abs, _mm256_andnot_ps(sign_bit, values));
    }

    __m128 max4 = _mm_max_ps(_mm256_castps256_ps128(max_abs), _mm256_extractf128_ps(max_abs, 1));
    ...
    for (; i + 32 <= n; i += 32) {
        __m256i i0 = roundf_i32_8(_mm256_mul_ps(_mm256_loadu_ps(x + i +  0), scale));
        __m256i i1 = roundf_i32_8(_mm256_mul_ps(_mm256_loadu_ps(x + i +  8), scale));
        __m256i i2 = roundf_i32_8(_mm256_mul_ps(_mm256_loadu_ps(x + i + 16), scale));
        __m256i i3 = roundf_i32_8(_mm256_mul_ps(_mm256_loadu_ps(x + i + 24), scale));

        sum32 = _mm256_add_epi32(sum32, _mm256_add_epi32(
        ...
        _mm256_storeu_si256((__m256i *)(dst + i), i0);
    }
    sum = hsum_i32_8(sum32);
#endif
    for (; i < n; i++) {
...
size_t quantize_i2_s(const float * src, void * dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    (void)quant_weights;
    GGML_ASSERT(nrow > 0);
    GGML_ASSERT(n_per_row > 0 && n_per_row % QK_I2_S == 0);
    ...
    memset(q, 0, packed_bytes + 32);

    for (int64_t row = 0; row < nrow; row++) {
        const float * src_row = src + row * n_per_row;
        uint8_t * dst_row = q + row * n_per_row / 4;
        for (int64_t block = 0; block < n_per_row / QK_I2_S; block++) {
            const float * src_block = src_row + block * QK_I2_S;
            uint8_t * dst_block = dst_row + block * 32;
            for (int gp = 0; gp < 32; gp++) {
                uint8_t packed = 0;
                for (int group = 0; group < 4; group++) {
                    const double value = src_block[group * 32 + gp];
                    const uint8_t code = fabs(value) < 1e-6 ? 1 : (value * max > 0 ? 2 : 0);
                    packed |= code << (6 - 2 * group);
                }
                dst_block[gp] = packed;
            }
        }
...
static void ggml_vec_dot_i2_i8_s_1x1(int n, float * s, size_t bs, const void * vx, size_t bx, const void * vy, size_t by, int nrc) {
    ...
    for (int row = 0; row < nrc; row++) {
        ...
        for (int i = 0; i < group32_num; i++) {
            ...
            for (int j = 0; j < 32; j++) {
                ...
                const __m256i sum16 = _mm256_add_epi16(
                    _mm256_add_epi16(xq8_0, xq8_1),
                    _mm256_add_epi16(xq8_2, xq8_3));
                accu = _mm256_add_epi32(accu, _mm256_madd_epi16(sum16, one16));
    ...
    for (int row = 0; row < nrc; row++) {
        int32_t sum = 0;
        const uint8_t * x_row = x + row * n / 4;
        for (int block = 0; block < n / QK_I2_S; block++) {
            const uint8_t * x_block = x_row + block * 32;
            const int8_t * y_block = y + block * QK_I2_S;
            for (int gp = 0; gp < 32; gp++) {
                const uint8_t packed = x_block[gp];
                sum += ((packed >> 6) & 0x03) * y_block[gp +  0];
                sum += ((packed >> 4) & 0x03) * y_block[gp + 32];
                sum += ((packed >> 2) & 0x03) * y_block[gp + 64];
                sum += ((packed >> 0) & 0x03) * y_block[gp + 96];
            }
        }
        s[row] = (float)sum;
    }
