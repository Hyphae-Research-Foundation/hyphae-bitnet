...
static void ggml_compute_forward_mul_mat_one_chunk(
    ...
    if (src0->type == GGML_TYPE_I2_S) {
        const float * post_scales = (const float *)((const char *)wdata + ggml_nelements(src1));
        const int32_t * act_sums = (const int32_t *)((const char *)post_scales + ggml_nrows(src1) * sizeof(float));
        ...
        for (int64_t iir1 = ir1_start; iir1 < ir1_end; iir1 += blck_1) {
            for (int64_t iir0 = ir0_start; iir0 < ir0_end; iir0 += blck_0) {
                for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir1_end; ++ir1) {
                    ...
                    const char * src0_row = (const char *)src0->data + (i02 * nb02 + i03 * nb03)/4;
                    const int64_t activation_row = (i13*ne12 + i12)*ne11 + i11;
                    const char * src1_col_de = (const char *)wdata + activation_row*ne10;
...
void ggml_compute_forward_mul_mat(
    ...
    const bool i2s_profile = src0->type == GGML_TYPE_I2_S && ggml_i2s_profile_enabled();
    const char * i2s_profile_name = ggml_get_name(src0);
    const int64_t i2s_profile_m = ne01;
    const int64_t i2s_profile_n = ne11 * ne12 * ne13;
    const int64_t i2s_profile_k = ne00;
    if (i2s_profile && ith == 0) {
        ggml_i2s_profile_record(
            GGML_I2S_PROFILE_OP, i2s_profile_name,
            i2s_profile_m, i2s_profile_n, i2s_profile_k, 1, 0);
    }
...
UseGgmlGemm1:;
    ...
    if (src1->type != vec_dot_type) {
        ...
        const int64_t quantize_start_us = i2s_profile ? ggml_time_us() : 0;
        ...
        // I2_S uses post-scales and sums after the complete quantized activation buffer.
        float * post_scales = NULL;
        int32_t * act_sums = NULL;
        if (src0->type == GGML_TYPE_I2_S) {
            post_scales = (float *)((char *)wdata + ggml_nelements(src1));
            act_sums = (int32_t *)((char *)post_scales + ggml_nrows(src1) * sizeof(float));
        }
    ...
    #else
        if (src0->type == GGML_TYPE_I2_S) {
            const int64_t n_act_rows = ggml_nrows(src1);
            const int64_t row_begin = n_act_rows * ith / nth;
            const int64_t row_end = n_act_rows * (ith + 1) / nth;
            const float weight_scale = *(const float *)((const uint8_t *)src0->data + ggml_nelements(src0) / 4);
            for (int64_t row = row_begin; row < row_end; row++) {
                ...
                quantize_row_i8_s(
                    ...
                    wdata + row * nbw1,
                    ne10, &activation_scale, act_sums + row);
                post_scales[row] = activation_scale > 0.0f ? weight_scale / activation_scale : 0.0f;
            }
            if (i2s_profile) {
                ggml_i2s_profile_record(
                    GGML_I2S_PROFILE_QUANTIZE, i2s_profile_name,
                    i2s_profile_m, i2s_profile_n, i2s_profile_k,
                    row_end - row_begin, ggml_time_us() - quantize_start_us);
            }
        } else {
            for (int64_t i13 = 0; i13 < ne13; ++i13) {
                for (int64_t i12 = 0; i12 < ne12; ++i12) {
    ...
    const int64_t barrier_start_us = i2s_profile ? ggml_time_us() : 0;
    ggml_barrier(params->threadpool);
    if (i2s_profile) {
        ggml_i2s_profile_record(
            GGML_I2S_PROFILE_BARRIER, i2s_profile_name,
            i2s_profile_m, i2s_profile_n, i2s_profile_k, 1, ggml_time_us() - barrier_start_us);
    }
    ...
    if (src1->type != vec_dot_type) {
        ...
        if (src0->type == GGML_TYPE_I2_S) {
            const float * i2s_post_scales = (const float *)((const char *)wdata + ggml_nelements(src1));
            const int32_t * i2s_act_sums = (const int32_t *)((const char *)i2s_post_scales + ggml_nrows(src1) * sizeof(float));
            const int64_t tinyblas_start_us = i2s_profile ? ggml_time_us() : 0;
            ...
            for (int64_t i13 = 0; i13 < ne13; i13++)
                for (int64_t i12 = 0; i12 < ne12; i12++)
                    if (!llamafile_sgemm_i2s(params,
                                             ne01, ne11, ne00,
                                             (const char *)src0->data + (i12/r2*nb02 + i13/r3*nb03)/4,
                                             nb01/4,
                                             (const char *)wdata + (i12*ne11 + i13*ne12*ne11)*row_size,
                                             row_size/ggml_type_size(vec_dot_type),
                                             (char *)dst->data + i12*nb2 + i13*nb3,
                                             nb1/ggml_type_size(dst->type),
                                             i2s_post_scales + (i13*ne12 + i12)*ne11,
                                             i2s_act_sums + (i13*ne12 + i12)*ne11,
                                             i2s_profile ? &postprocess_us : NULL,
                                             i2s_profile ? &postprocessed_values : NULL))
                        goto UseGgmlGemm2;
            if (i2s_profile) {
                const int64_t tinyblas_us = ggml_time_us() - tinyblas_start_us - postprocess_us;
                ggml_i2s_profile_record(
                    GGML_I2S_PROFILE_TINYBLAS, i2s_profile_name,
                    i2s_profile_m, i2s_profile_n, i2s_profile_k, ith == 0 ? 1 : 0, tinyblas_us);
                ggml_i2s_profile_record(
                    GGML_I2S_PROFILE_POSTPROCESS, i2s_profile_name,
                    i2s_profile_m, i2s_profile_n, i2s_profile_k, postprocessed_values, postprocess_us);
            }
            return;
        }
...
UseGgmlGemm2:;
    ...
    if (src0->type == GGML_TYPE_I2_S && ggml_n_dims(src0) == 2 && ne12 == 1 && ne13 == 1) {
    ...
    const size_t src1_col_stride = src1_cont || src1->type != vec_dot_type ? i2s_row_size : nb11;
        ...
        const float * post_scales = (const float *)((const char *)src1_wdata + ggml_nelements(src1));
        const int32_t * act_sums = (const int32_t *)((const char *)post_scales + ggml_nrows(src1) * sizeof(float));
        ...
        if (ne11 >= 4) {
            ...
            for (int64_t col_start = 0; col_start < n_gemm_cols; col_start += 4) {
                ...
                for (int64_t col = 0; col < 4; col++) {
                    const int64_t postprocess_start_us = i2s_profile ? ggml_time_us() : 0;
        ...
        if (i2s_profile) {
            const int64_t fallback_us = MAX(0, ggml_time_us() - fallback_start_us - postprocess_us);
            ggml_i2s_profile_record(
                GGML_I2S_PROFILE_FALLBACK, i2s_profile_name,
                i2s_profile_m, i2s_profile_n, i2s_profile_k, ith == 0 ? 1 : 0, fallback_us);
            ggml_i2s_profile_record(
                GGML_I2S_PROFILE_POSTPROCESS, i2s_profile_name,
                i2s_profile_m, i2s_profile_n, i2s_profile_k, postprocessed_values, postprocess_us);
        }
    ...
    while (current_chunk < nchunk0 * nchunk1) {
        ...
        if (i2s_profile) {
            ggml_i2s_profile_record(
                GGML_I2S_PROFILE_FALLBACK, i2s_profile_name,
                i2s_profile_m, i2s_profile_n, i2s_profile_k,
                ith == 0 ? 1 : 0, ggml_time_us() - fallback_start_us);
        }
                ...
                case GGML_OP_MUL_MAT:
                    {
                        ...
                        if (node->src[1]->type != vec_dot_type) {
                            if (vec_dot_type == GGML_TYPE_I8_S) {
                                ...
                                cur = ggml_nelements(node->src[1])
                                    + ggml_nrows(node->src[1]) * sizeof(float)
                                    + ggml_nrows(node->src[1]) * sizeof(int32_t);
                            } else {
