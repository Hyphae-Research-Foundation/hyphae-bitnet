...
static void q1_gemm_generic_acc_tiles(
        const block_q1_0x4 * GGML_RESTRICT b_ptr,
        const block_q8_0x4 * const *       a_tiles,
        int                                n_tiles,
        int                                nb,
        float                              sumf[][4][4]) {
    const int qk       = QK1_0;
    ...
    for (int l = 0; l < nb; l++) {
        ...
        for (int k = 0; k < qk / blocklen; ++k) {
            const uint8_t bits0 = b_ptr[l].qs[k * 4 + 0];
            const uint8_t bits1 = b_ptr[l].qs[k * 4 + 1];
            const uint8_t bits2 = b_ptr[l].qs[k * 4 + 2];
            const uint8_t bits3 = b_ptr[l].qs[k * 4 + 3];
            const int q_offset = (k % (QK8_0 / blocklen)) * 4 * blocklen;
            const int a_off    = 4 * l + k / (QK8_0 / blocklen);
            for (int p = 0; p < blocklen; ++p) {
                const int s0 = (bits0 & (1u << p)) ? 1 : -1;
                const int s1 = (bits1 & (1u << p)) ? 1 : -1;
                const int s2 = (bits2 & (1u << p)) ? 1 : -1;
                const int s3 = (bits3 & (1u << p)) ? 1 : -1;
                for (int t = 0; t < n_tiles; t++) {
                    const block_q8_0x4 * GGML_RESTRICT a_blk = a_tiles[t] + a_off;
                    const float a_d[4] = {
                    ...
                    for (int m = 0; m < 4; ++m) {
                        sumf[t][m][0] += (float) (s0 * q_row[m]) * d0[0] * a_d[m];
                        sumf[t][m][1] += (float) (s1 * q_row[m]) * d0[1] * a_d[m];
                        sumf[t][m][2] += (float) (s2 * q_row[m]) * d0[2] * a_d[m];
                        sumf[t][m][3] += (float) (s3 * q_row[m]) * d0[3] * a_d[m];
                    }
                }
            }
        }
...
void ggml_gemm_q1_0_4x8_q8_0_generic(int                        n,
    ...
    /* One 4x8 weight panel applied to 8 (then 4) activation rows. */
    const int act_tile = celiums_exact_q1_gemm_act_tile_rows(nr);
    int y = 0;
    for (; act_tile >= 8 && y + 8 <= nr; y += 8) {
        const block_q8_0x4 * a_tiles[2] = {
            (const block_q8_0x4 *) vy + (y * nb),
            (const block_q8_0x4 *) vy + (y * nb) + (4 * nb),
        };
        for (int x = 0; x < nc / ncols_interleaved; x++) {
            const block_q1_0x4 * b_ptr = (const block_q1_0x4 *) vx + (x * nb);
            float sumf[2][4][4];
            q1_gemm_generic_acc_tiles(b_ptr, a_tiles, 2, nb, sumf);
            for (int t = 0; t < 2; t++) {
                for (int m = 0; m < 4; m++) {
                    for (int j = 0; j < ncols_interleaved; j++) {
                        s[(y + t * 4 + m) * bs + x * ncols_interleaved + j] = sumf[t][m][j];
                    }
                }
            }
        }
    ...
    for (; y + 4 <= nr; y += 4) {
        const block_q8_0x4 * a_tiles[1] = {
            (const block_q8_0x4 *) vy + (y * nb),
        };
        for (int x = 0; x < nc / ncols_interleaved; x++) {
            const block_q1_0x4 * b_ptr = (const block_q1_0x4 *) vx + (x * nb);
            float sumf[1][4][4];
            q1_gemm_generic_acc_tiles(b_ptr, a_tiles, 1, nb, sumf);
            for (int m = 0; m < 4; m++) {
                for (int j = 0; j < ncols_interleaved; j++) {
                    s[(y + m) * bs + x * ncols_interleaved + j] = sumf[0][m][j];
                }
            }
        }
    }
...
static bool q1_repack_expand_to_q8(void) {
    return ggml_cpu_has_neon() && ggml_cpu_has_matmul_int8();
}

static size_t q1_expanded_row_bytes(const struct ggml_tensor * t) {
    return (size_t) (t->ne[0] / QK1_0) * sizeof(block_q8_0x4);
}

static size_t q1_expanded_nbytes(const struct ggml_tensor * t) {
    return q1_expanded_row_bytes(t) * (size_t) ggml_nrows(t);
}

static block_q8_0x4 q1_rows_slice_to_q8_0x4(const block_q1_0 in[4], int slice) {
    block_q8_0 tmp[4];
    for (int row = 0; row < 4; ++row) {
        tmp[row].d = in[row].d;
        const uint8_t * bits = in[row].qs + slice * 4;
        for (int b = 0; b < 4; ++b) {
    ...
    return make_block_q8_0x4(tmp, 8);
}

static int repack_q1_0_to_q8_0_4x8(struct ggml_tensor *       t,
                                   const void * GGML_RESTRICT data,
                                   size_t                     data_size) {
    constexpr int nrows_interleaved = 4;
    block_q8_0x4 *     dst     = (block_q8_0x4 *) t->data;
    ...
    for (int b = 0; b < nrow; b += nrows_interleaved) {
        for (int64_t x = 0; x < nblocks; x++) {
            block_q1_0 rows[4];
            for (int i = 0; i < nrows_interleaved; i++) {
                rows[i] = src[x + (int64_t) i * nblocks];
            }
            for (int slice = 0; slice < QK1_0 / QK8_0; ++slice) {
                *dst++ = q1_rows_slice_to_q8_0x4(rows, slice);
            }
        }
        src += (int64_t) nrows_interleaved * nblocks;
    }

    t->nb[1] = q1_expanded_row_bytes(t);
    for (int dim = 2; dim < GGML_MAX_DIMS; ++dim) {
        t->nb[dim] = t->nb[dim - 1] * (size_t) t->ne[dim - 1];
    }
    return 0;
}
...
static int repack_q1_0_to_q1_0_4_bl(struct ggml_tensor *       t,
    ...
    GGML_ASSERT(interleave_block == 8);
    if (q1_repack_expand_to_q8()) {
        return repack_q1_0_to_q8_0_4x8(t, data, data_size);
    }
    ...
    for (int b = 0; b < nrow; b += nrows_interleaved) {
        for (int64_t x = 0; x < nblocks; x++) {
            ...
            celiums_exact_q1_make_4x8(&dst_tmp[0], &dst_tmp[1], &dst_tmp[2], &dst_tmp[3], dst);
            ++dst;
        }
        src += (int64_t) nrows_interleaved * nblocks;
    }
...
static size_t ggml_backend_cpu_repack_buffer_type_get_alloc_size(ggml_backend_buffer_type_t buft, const struct ggml_tensor * tensor) {
    GGML_UNUSED(buft);
    if (tensor->type == GGML_TYPE_Q1_0 && q1_repack_expand_to_q8() &&
            ggml_repack_get_optimal_repack_type(tensor) != nullptr) {
        return q1_expanded_nbytes(tensor);
    }
